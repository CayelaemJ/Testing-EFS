...
function rateLimit(reply, key, limit, windowMs) {
    const now = Date.now();
...
app.addHook("onRequest", async (req, reply) => {
    ...
    if (!rateLimit(reply, `${authPath ? "auth" : "api"}:${ip}`, authPath ? AUTH_RATE_LIMIT : RATE_LIMIT, authPath ? AUTH_WINDOW_MS : RATE_WINDOW_MS))
        return;
    ...
    if (req.method !== "GET" && req.method !== "HEAD" && path.startsWith("/api/")) {
        ...
        if (!authHeader && (origin || referer)) {
            ...
            const suppliedOrigin = origin || (() => { try {
...
app.setErrorHandler((error, req, reply) => {
    ...
    if (status >= 400 && status < 500)
        return reply.code(status).send({ error: error.message || "request rejected" });
...
function dashboardQueryError(query) {
    const current = currentPeriod();
    if (query.period && !/^\d{4}-(0[1-9]|1[0-2])$/.test(query.period))
        return "period must be YYYY-MM";
    if (query.period && query.period > current)
        return `period cannot be later than ${current}`;
    if (query.quarter && !/^\d{4}-Q[1-4]$/.test(query.quarter))
        return "quarter must be YYYY-Q1..YYYY-Q4";
    if (query.range && !VALID_DASHBOARD_RANGES.has(query.range))
        return "range must be 30d, quarter, all or month";
    if ([query.period, query.quarter, query.range].filter((v) => v != null).length > 1)
        return "use period, quarter, or range — not more than one";
    if (!query.period && query.range === "month")
        return "range=month requires period=YYYY-MM";
    if (query.income && query.income !== "all" && !VALID_INCOME_BANDS.has(query.income))
        return "income is not a supported income-band code";
    return null;
}
const serveHtml = async (reply, file) => reply
...
function redactAuditDetail(value) {
    ...
    for (const [key, val] of Object.entries(value)) {
        if (/password|token|secret|authorization|smtp/i.test(key))
            out[key] = "[REDACTED]";
        else
            out[key] = redactAuditDetail(val);
    }
    return out;
}
// ── public pages ──
...
app.get("/admin", async (req, reply) => {
    ...
    if (!canAccessModule(user, "admin"))
        return reply.code(403).type("text/html").send("<h2 style='font-family:sans-serif;padding:40px'>Admins only. <a href='/dashboard'>Go to dashboard</a></h2>");
...
const COOKIE = { httpOnly: true, sameSite: "lax", secure: COOKIE_SECURE, path: "/", maxAge: 30 * 86400 };
...
app.post("/api/auth/set-password", async (req, reply) => {
    try {
        ...
        if (!password || password.length < 12)
            return reply.code(400).send({ error: "password must be at least 12 characters and include upper-case, lower-case, and numeric characters" });
...
const forgotPasswordThrottle = new Map();
app.post("/api/auth/forgot-password", async (req, reply) => {
    ...
    if (Date.now() - last < 60_000)
        return { ok: true }; // silently no-op if requested <60s ago
    forgotPasswordThrottle.set(email, Date.now());
    try {
        await requestPasswordReset(email);
    }
    catch { /* never leak errors here */ }
    return { ok: true };
});
...
app.post("/api/users", async (req, reply) => {
    ...
    try {
        ...
        logAdminAction(admin, "user.create", `Created user ${b.name} (${b.email}), role ${b.role}`, { targetType: "User", targetId: created?.id });
        return created;
    }
    catch (e) {
        return reply.code(400).send({ error: e.message });
...
app.post("/api/users/me/deactivate", async (req, reply) => {
    const user = await requireUser(req, reply);
    if (!user)
        return;
...
app.post("/api/analytics/event", async (req, reply) => {
    ...
    const b = req.body ?? {};
    if (!b.type || !b.path)
        return reply.code(400).send({ error: "type and path are required" });
    try {
        await recordEvent({ userId: user?.id ?? null, type: b.type, path: b.path, value: b.value, partnerId: user?.partnerId ?? null });
        return { ok: true };
    }
    catch {
        return { ok: true };
...
app.get("/api/admin/analytics/summary", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
...
app.get("/api/admin/sections", async (req, reply) => {
    ...
    return sections.map((s) => ({
        ...
        overrides: s.overrides.map((o) => ({ userId: o.userId, name: o.user.name, email: o.user.email })),
    }));
});
app.patch("/api/admin/sections/:key", async (req, reply) => {
    const admin = await requireAdmin(req, reply);
    if (!admin)
        return;
    try {
        ...
        return reply.code(400).send({ error: e?.message || "could not update section" });
    }
});
app.post("/api/admin/sections/:key/grant", async (req, reply) => {
    const admin = await requireAdmin(req, reply);
    if (!admin)
        return;
    if (!req.body?.userId)
...
app.post("/api/admin/sections/:key/revoke", async (req, reply) => {
    const admin = await requireAdmin(req, reply);
    if (!admin)
        return;
    if (!req.body?.userId)
...
app.get("/api/admin/reports/:key/template", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
...
app.post("/api/admin/batches/:batchId/commit", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
    const jobId = startCommitJob(req.params.batchId);
    return reply.code(202).send({ status: "ACCEPTED", jobId, batchId: req.params.batchId });
});
app.get("/api/admin/commit-jobs/:jobId", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
    const job = getCommitJob(req.params.jobId);
    if (!job)
        return reply.code(404).send({ error: "unknown commit job" });
    if (job.status === "PENDING")
        return { status: "PROCESSING" };
    if (job.status === "FAILED")
        return { status: "FAILED", error: job.error };
...
app.post("/api/admin/batches/:batchId/revert", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
    try {
...
app.post("/api/admin/reset-all", async (req, reply) => {
    const admin = await requireAdmin(req, reply);
    if (!admin)
        return;
...
app.post("/api/admin/integration", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
    const cfg = await saveSyncConfig(req.body || {});
    return publicSyncConfig(cfg);
});
app.post("/api/admin/integration/test", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
    return testSyncConnection((req.body || {}));
});
...
app.get("/api/admin/sync-jobs/:jobId", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
    const job = getSyncJob(req.params.jobId);
    if (!job)
        return reply.code(404).send({ error: "unknown sync job" });
    if (job.status === "PENDING")
        return { status: "PROCESSING" };
    if (job.status === "FAILED")
        return { status: "FAILED", error: job.error };
...
app.post("/api/admin/email-settings", async (req, reply) => {
    const admin = await requireAdmin(req, reply);
    if (!admin)
        return;
    try {
        ...
        return reply.code(400).send({ error: e?.message || "Could not save email settings" });
    }
});
app.post("/api/admin/email-settings/test", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
    try {
        return await testEmailConnection(req.body || {});
    }
    catch (e) {
        return reply.code(400).send({ error: e?.message || "Email test failed" });
    }
});
// manual "run now" for automations — lets an admin verify config without waiting for the hourly tick
app.post("/api/admin/automations/run", async (req, reply) => {
    const admin = await requireAdmin(req, reply);
    if (!admin)
        return;
    try {
        ...
        return reply.code(400).send({ error: e?.message || "Could not run automation" });
    }
});
...
app.post("/api/report-schedules", async (req, reply) => {
    ...
    if (!user)
        return;
    try {
        return await createReportSchedule(user, req.body);
    }
    catch (e) {
        return reply.code(400).send({ error: e?.message || "Could not create report schedule" });
    }
});
app.patch("/api/report-schedules/:id", async (req, reply) => {
    const user = await requireUser(req, reply);
    if (!user)
        return;
    try {
        return await updateReportSchedule(user, req.params.id, req.body || {});
    }
    catch (e) {
        return reply.code(400).send({ error: e?.message || "Could not update report schedule" });
    }
});
app.delete("/api/report-schedules/:id", async (req, reply) => {
    const user = await requireUser(req, reply);
    if (!user)
        return;
    try {
        return await deleteReportSchedule(user, req.params.id);
    }
    catch (e) {
        return reply.code(400).send({ error: e?.message || "Could not delete report schedule" });
    }
});
app.post("/api/report-schedules/:id/send-now", async (req, reply) => {
    const user = await requireUser(req, reply);
    if (!user)
        return;
    try {
        return await sendReportNow(user, req.params.id);
    }
    catch (e) {
        return reply.code(400).send({ error: e?.message || "Could not send report" });
    }
});
...
app.post("/api/admin/partners", async (req, reply) => {
    const admin = await requireAdmin(req, reply);
    if (!admin)
        return;
    if (!req.body?.name?.trim())
        return reply.code(400).send({ error: "partner name required" });
    const created = await createPartner(req.body);
    logAdminAction(admin, "partner.create", `Created channel partner "${req.body.name}"`, { targetType: "Partner", targetId: created?.id });
    return created;
});
app.put("/api/admin/partners/:id", async (req, reply) => {
    const admin = await requireAdmin(req, reply);
    if (!admin)
        return;
...
app.delete("/api/admin/partners/:id", async (req, reply) => {
    const admin = await requireAdmin(req, reply);
    if (!admin)
        return;
...
app.post("/api/admin/partners/:id/assign", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
    if (req.body?.userId)
        return assignUserToPartner(req.body.userId, req.params.id);
    if (req.body?.employerId)
        return assignEmployerToPartner(req.body.employerId, req.params.id);
    return reply.code(400).send({ error: "userId or employerId required" });
});
// public: theme by slug (for the future branded login page)
app.get("/api/partner-theme/:slug", async (req) => {
    return themeForSlug(req.params.slug);
});
// ── dashboard: real stock/as-at + flow/in-window filtering ──
app.get("/api/employers/:employerId/dashboard", async (req, reply) => {
    const user = await requireUser(req, reply);
    if (!user)
        return;
    const { employerId } = req.params;
    if (!canViewEmployer(user, employerId))
        return reply.code(403).send({ error: "no access to this employer" });
    const queryError = dashboardQueryError(req.query);
    if (queryError)
        return reply.code(400).send({ error: queryError });
    const payload = await getDashboardPayload(employerId, {
    ...
    if (!sections.voiceOfEmployee)
        payload.chat = { available: false };
    return payload;
});
// ── available months (periods) for the month picker, scoped to access ──
app.get("/api/employers/:employerId/periods", async (req, reply) => {
    const user = await requireUser(req, reply);
    if (!user)
        return;
    if (!canViewEmployer(user, req.params.employerId))
...
app.post("/api/employers/:employerId/snapshot", async (req, reply) => {
    if (!(await requireAdmin(req, reply)))
        return;
    const result = await snapshotEmployer(req.params.employerId, req.body.period);
    if (result.persisted)
        notifyScoreChangeIfCurrentPeriod(req.params.employerId, result.period, currentPeriod()).catch(() => { });
    return result;
});
...
function startSyncScheduler(app) {
...
app.get("/api/portfolio", async (req, reply) => {
    ...
    if (!user)
        return;
    if (!canAccessModule(user, "portfolio"))
        return reply.code(403).send({ error: "no portfolio access" });
    const queryError = dashboardQueryError(req.query);
    if (queryError)
        return reply.code(400).send({ error: queryError });
    const ids = allowedEmployerIds(user);
    ...
    for (const employer of employers) {
        const payload = await getDashboardPayload(employer.id, req.query);
...
app.get("/api/dashboard/first", async (req, reply) => {
    const user = await requireUser(req, reply);
    if (!user)
        return;
    const queryError = dashboardQueryError(req.query);
    ...
    if (!employer)
        return reply.code(404).send({ error: "no employer data available" });
    const payload = await getDashboardPayload(employer.id, req.query);
    const sections = await sectionsForUser(user);
    if (!sections.voiceOfEmployee)
        payload.chat = { available: false };
