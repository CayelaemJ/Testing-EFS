...
function alertEmailList(cfg) {
    return String(cfg.alertEmails || "").split(",").map((s) => s.trim()).filter(Boolean);
}
...
export async function runStaleAccountCheck() {
    const cfg = await config();
    const days = cfg.staleDeactivateDays;
    if (!days || days <= 0)
        return { ran: false };
    ...
    await prisma.systemEmailConfig.update({ where: { id: "default" }, data: { lastStaleCheckAt: new Date() } });
    if (stale.length) {
        const rows = stale.map((u) => `<li>${u.name} — ${u.email}</li>`).join("");
        await notifyAdmins({
...
export async function runWeeklyDigestIfDue() {
    ...
    if (!cfg.digestEnabled)
        return { ran: false };
    const last = cfg.lastDigestSentAt ? new Date(cfg.lastDigestSentAt).getTime() : 0;
    if (Date.now() - last < 7 * 864e5)
        return { ran: false };
    const s = await engagementSummary(7);
    const fmtDuration = (sec) => sec == null ? "—" : sec < 60 ? `${sec}s` : `${Math.round(sec / 60)}m`;
    const topPages = s.topPages.slice(0, 5).map((p) => `<li>${p.path} — ${p.views} view(s)</li>`).join("") || "<li>No pageviews this week.</li>";
    const topUsers = s.byUser.slice(0, 5).map((u) => `<li>${u.name} — ${u.logins} login(s), ${u.pageviews} pageview(s)</li>`).join("") || "<li>No activity this week.</li>";
    await notifyAdmins({
    ...
    await prisma.systemEmailConfig.update({ where: { id: "default" }, data: { lastDigestSentAt: new Date() } });
    return { ran: true };
}
...
export async function notifyScoreChangeIfCurrentPeriod(employerId, period, currentPeriod) {
    const cfg = await config();
    if (!cfg.scoreChangeAlertsEnabled)
    ...
    if (!latest || !previous)
        return { notified: false }; // need two points to call it a "change"
