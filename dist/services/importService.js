...
async function chunkedRowWriter(batchId) {
    ...
    return async (rows) => {
        ...
        await prisma.importBatchRow.createMany({
            data: rows.map((data, i) => ({ batchId, rowIndex: offset + i, data: data })),
        });
        offset += rows.length;
    };
}
async function loadStagedRows(batch) {
...
export async function uploadAndValidate(opts) {
    ...
    const updated = await prisma.importBatch.update({
        ...
        data: {
            ...
            errors: result.ok
                ? undefined
                : { cellErrors: result.errors, missingColumns: result.missingColumns, unknownColumns: result.unknownColumns },
        },
    });
...
async function commitRowsChunk(tx, reportKey, rows, stats, batchId) {
    ...
    switch (reportKey) {
        ...
        case "employees": {
            ...
            for (const row of rows) {
                if (row.site_name)
                    siteKeys.set(`${row.employer_ref}|${row.site_name}`, { employerId: row.employer_ref, name: row.site_name });
            }
            ...
            for (const row of rows) {
                ...
                else if (compareDateValues(row.observed_at, existing.observedAt, "observed_at") > 0
        ...
        case "platform_users": {
            ...
            for (const row of rows) {
                ...
                if (!employeeId)
                    throw new Error(`platform_users references missing employee ${row.employer_ref}/${row.payroll_ref}`);
        ...
        case "journeys": {
            ...
            for (const row of rows) {
                ...
                if (!platformUserId)
                    throw new Error(`journeys references missing platform user ${row.employer_ref}/${row.payroll_ref}`);
        ...
        case "debt_accounts": {
            ...
            for (const row of rows) {
                const platformUserId = platformUserIds.get(`${row.employer_ref}|${row.payroll_ref}`);
                if (!platformUserId)
                    throw new Error(`debt_accounts references missing platform user ${row.employer_ref}/${row.payroll_ref}`);
        ...
        case "policies": {
            ...
            for (const row of rows) {
                const platformUserId = platformUserIds.get(`${row.employer_ref}|${row.payroll_ref}`);
                if (!platformUserId)
                    throw new Error(`policies references missing platform user ${row.employer_ref}/${row.payroll_ref}`);
        ...
        case "ratings": {
            ...
            for (const row of rows) {
                const platformUserId = platformUserIds.get(`${row.employer_ref}|${row.payroll_ref}`);
                if (!platformUserId)
                    throw new Error(`ratings references missing platform user ${row.employer_ref}/${row.payroll_ref}`);
        ...
        case "referrals": {
            ...
            for (const row of rows) {
                const platformUserId = platformUserIds.get(`${row.employer_ref}|${row.payroll_ref}`);
                if (!platformUserId)
                    throw new Error(`referrals references missing platform user ${row.employer_ref}/${row.payroll_ref}`);
        ...
        case "salary_advances": {
            ...
            for (const row of rows) {
                const employeeId = employeeIds.get(`${row.employer_ref}|${row.payroll_ref}`);
                if (!employeeId)
                    throw new Error(`salary_advances references missing employee ${row.employer_ref}/${row.payroll_ref}`);
    ...
    if (batch.reportKey === "employers") {
        ...
        await prisma.$transaction(async (tx) => {
            for (const row of stagedRows) {
                ...
                const dependencies = Object.values(employer._count || {}).reduce((sum, value) => sum + Number(value || 0), 0);
                if (dependencies > 0) {
            ...
            if (employerRefs.length)
                await tx.employer.deleteMany({ where: { id: { in: employerRefs } } });
