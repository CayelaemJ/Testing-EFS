...
function withEnv(cfg, draft = {}) {
    const pick = (draftValue, envValue, savedValue) => draftValue !== undefined ? draftValue : envValue !== undefined ? envValue : savedValue;
    const draftString = (key) => draft[key] === undefined ? undefined : clean(draft[key]);
    const envString = (name) => process.env[name] === undefined ? undefined : clean(process.env[name]);
    const envPort = process.env.SMTP_PORT ? Number(process.env.SMTP_PORT) : undefined;
    return {
        emailProvider: pick(draft.emailProvider, (envString("EMAIL_PROVIDER") || "smtp"), cfg.emailProvider || "smtp"),
        resendApiKey: pick(draftString("resendApiKey"), envString("RESEND_API_KEY"), undefined),
        resendFromEmail: pick(draftString("resendFromEmail"), envString("RESEND_FROM_EMAIL"), undefined),
        smtpHost: pick(draftString("smtpHost"), envString("SMTP_HOST"), cfg.smtpHost),
        ...
        smtpUsername: pick(draftString("smtpUsername"), envString("SMTP_USERNAME"), cfg.smtpUsername),
        smtpPassword: pick(draftString("smtpPassword"), envString("SMTP_PASSWORD"), cfg.smtpPassword),
        fromName: pick(draftString("fromName"), envString("SMTP_FROM_NAME"), cfg.fromName),
        fromEmail: pick(draftString("fromEmail"), envString("SMTP_FROM_EMAIL"), cfg.fromEmail),
        replyTo: pick(draftString("replyTo"), envString("SMTP_REPLY_TO"), cfg.replyTo),
        defaultTimezone: pick(draftString("defaultTimezone"), envString("REPORT_TIMEZONE"), cfg.defaultTimezone),
        portalBaseUrl: pick(draftString("portalBaseUrl"), envString("PORTAL_BASE_URL"), cfg.portalBaseUrl),
...
export async function publicEmailConfig() {
    ...
    return {
        ...
        alertSlackWebhookUrl: cfg.alertSlackWebhookUrl,
        digestEnabled: cfg.digestEnabled,
        lastDigestSentAt: cfg.lastDigestSentAt,
        staleDeactivateDays: cfg.staleDeactivateDays,
        lastStaleCheckAt: cfg.lastStaleCheckAt,
        scoreChangeAlertsEnabled: cfg.scoreChangeAlertsEnabled,
    };
}
export async function saveEmailConfig(input) {
    ...
    if ("emailProvider" in input) {
        if (input.emailProvider !== "smtp" && input.emailProvider !== "resend")
            throw new Error("email provider must be SMTP or Resend");
        data.emailProvider = input.emailProvider;
    }
    if ("resendApiKey" in input && clean(input.resendApiKey))
        data.resendApiKey = clean(input.resendApiKey);
    if ("resendFromEmail" in input)
        data.resendFromEmail = clean(input.resendFromEmail);
    if ("smtpHost" in input)
        data.smtpHost = clean(input.smtpHost);
    if ("smtpPort" in input && input.smtpPort != null)
        data.smtpPort = Math.max(1, Math.min(65535, Number(input.smtpPort)));
    if ("smtpSecure" in input)
        data.smtpSecure = Boolean(input.smtpSecure);
    if ("smtpRequireTls" in input)
        data.smtpRequireTls = Boolean(input.smtpRequireTls);
    if ("smtpRejectUnauthorized" in input)
        data.smtpRejectUnauthorized = Boolean(input.smtpRejectUnauthorized);
    if ("smtpUsername" in input)
        data.smtpUsername = clean(input.smtpUsername);
    if (clean(input.smtpPassword))
        data.smtpPassword = clean(input.smtpPassword);
    if ("fromName" in input)
        data.fromName = clean(input.fromName) || "empower-fin Dashboard Portal";
    if ("fromEmail" in input)
        data.fromEmail = clean(input.fromEmail);
    if ("replyTo" in input)
        data.replyTo = clean(input.replyTo);
    if ("defaultTimezone" in input) {
        ...
        if (!DateTime.now().setZone(zone).isValid)
            throw new Error("default timezone is not a valid IANA timezone");
        data.defaultTimezone = zone;
    }
    if ("portalBaseUrl" in input)
        data.portalBaseUrl = clean(input.portalBaseUrl);
    if ("alertEmails" in input)
        data.alertEmails = clean(input.alertEmails);
    if ("alertSlackWebhookUrl" in input)
        data.alertSlackWebhookUrl = clean(input.alertSlackWebhookUrl);
    if ("digestEnabled" in input)
        data.digestEnabled = Boolean(input.digestEnabled);
    ...
    if ("scoreChangeAlertsEnabled" in input)
        data.scoreChangeAlertsEnabled = Boolean(input.scoreChangeAlertsEnabled);
    if (data.fromEmail && !emailOk(data.fromEmail))
        throw new Error("from email is not valid");
    if (data.replyTo && !emailOk(data.replyTo))
        throw new Error("reply-to email is not valid");
    if (data.resendFromEmail && !emailOk(data.resendFromEmail))
        throw new Error("Resend From email is not valid");
    await prisma.systemEmailConfig.update({ where: { id: current.id }, data });
    return publicEmailConfig();
}
async function smtpTransport(draft = {}) {
    ...
    if (!cfg.fromEmail || !emailOk(cfg.fromEmail))
        throw new Error("A valid From email is required");
    if (cfg.smtpUsername && !cfg.smtpPassword)
        throw new Error("SMTP password is required when an SMTP username is supplied");
...
async function sendEmail(opts, draft = {}) {
    const saved = await getEmailConfig();
    const cfg = withEnv(saved, draft);
    ...
    if (cfg.emailProvider === "resend") {
        ...
        if (!fromEmail || !emailOk(fromEmail))
            throw new Error("A valid Resend From email is required");
...
export async function portalBaseUrl() {
    const cfg = await publicEmailConfig();
    return clean(cfg.portalBaseUrl) || null;
}
export async function testEmailConnection(draft) {
    const recipient = clean(draft.recipient);
    if (!recipient || !emailOk(recipient))
    ...
    try {
        ...
        if (cfg.emailProvider === "smtp")
            await (await smtpTransport(draft)).transporter.verify();
        await sendEmail({
            to: recipient,
...
export function nextRunFor(input, after = new Date()) {
    ...
    if (input.frequency === "ONCE") {
        ...
        if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date))
            throw new Error("send date is required for a one-off report");
        next = DateTime.fromISO(`${date}T${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}`, { zone });
        if (!next.isValid)
            throw new Error("send date is invalid");
        if (next <= localNow)
            throw new Error("send date/time must be in the future");
    }
        ...
        if (!Number.isInteger(weekday) || weekday < 1 || weekday > 7)
            throw new Error("weekday must be Monday (1) to Sunday (7)");
        ...
        const inMonth = (base) => base.set({ day: Math.min(wanted, base.daysInMonth || wanted), hour, minute, second: 0, millisecond: 0 });
        next = inMonth(localNow);
        if (next <= localNow)
            next = inMonth(localNow.plus({ months: 1 }).startOf("month"));
    }
    else {
...
function normalizeRecipients(input, fallbackEmail, allowAdditional) {
    ...
    if (!recipients.length)
        recipients.push(fallbackEmail.toLowerCase());
    const unique = [...new Set(recipients)];
    if (unique.length > 10)
        throw new Error("a report can be sent to a maximum of 10 recipients");
...
function cleanFilters(input) {
    const filters = {};
    if (input?.period) {
        if (!/^\d{4}-\d{2}$/.test(String(input.period)))
            throw new Error("period must be YYYY-MM");
        filters.period = String(input.period);
    }
    else {
        const range = ["30d", "quarter", "all"].includes(String(input?.range)) ? String(input.range) : "all";
        filters.range = range;
    }
    if (input?.site && input.site !== "all")
        filters.site = String(input.site);
    if (input?.income && input.income !== "all")
        filters.income = String(input.income);
    return filters;
}
export async function createReportSchedule(user, input) {
    const mail = await publicEmailConfig();
    if (!mail.configured)
        throw new Error("Email delivery is not configured. Ask an administrator to complete System Email Setup first.");
    const employer = await prisma.employer.findUnique({ where: { id: input.employerId }, select: { id: true, name: true } });
    if (!employer)
        throw new Error("employer not found");
    const canSee = user.role === "ADMIN" || (user.links || []).some((link) => link.employerId === employer.id);
    if (!canSee)
        throw new Error("you do not have access to this employer");
    ...
    const normalized = { ...input, frequency, timezone, sendTime };
...
export async function listReportSchedules(user, all = false) {
...
export async function updateReportSchedule(user, id, patch) {
    ...
    if (user.role !== "ADMIN" && current.userId !== user.id)
        throw new Error("you cannot edit this schedule");
...
export async function deleteReportSchedule(user, id) {
    const current = await prisma.reportSchedule.findUnique({ where: { id } });
    if (!current)
        return { deleted: true };
    if (user.role !== "ADMIN" && current.userId !== user.id)
        throw new Error("you cannot delete this schedule");
...
function reportHtml(payload, schedule, theme) {
    ...
    const completed = Array.isArray(payload.outcomes) ? payload.outcomes.reduce((sum, row) => sum + (Number(row.count) || 0), 0) : 0;
    const outcomeRows = (payload.outcomes || []).map((row) => `<tr><td style="padding:9px 8px;border-bottom:1px solid #eee">${escapeHtml(row.name)}</td><td style="padding:9px 8px;border-bottom:1px solid #eee;text-align:right">${Number(row.count || 0).toLocaleString("en-ZA")}</td><td style="padding:9px 8px;border-bottom:1px solid #eee;text-align:right;font-weight:700">${escapeHtml(row.stat)} ${escapeHtml(row.statL)}</td></tr>`).join("");
    const drivers = (payload.wellness?.drivers || []).map((row) => `<tr><td style="padding:7px 8px">${escapeHtml(row.name)}</td><td style="padding:7px 8px;text-align:right;font-weight:700">${row.available ? escapeHtml(row.score) : "Data not loaded"}</td></tr>`).join("");
    const warnings = payload.dataQuality?.warnings || [];
    const portalBase = clean(schedule.portalBaseUrl);
...
async function sendSchedule(scheduleId, triggeredBy) {
    ...
    const payload = await getDashboardPayload(schedule.employerId, filters);
    ...
    try {
        ...
        await prisma.reportDeliveryLog.create({ data: { scheduleId: schedule.id, triggeredBy, status: "SENT", recipients: recipients, subject, filters: filters } });
        ...
        await prisma.reportDeliveryLog.create({ data: { scheduleId: schedule.id, triggeredBy, status: "FAILED", recipients: recipients, subject, filters: filters, error: message } });
...
export async function sendReportNow(user, id) {
    const schedule = await prisma.reportSchedule.findUnique({ where: { id } });
    if (!schedule)
        throw new Error("schedule not found");
    if (user.role !== "ADMIN" && schedule.userId !== user.id)
        throw new Error("you cannot send this schedule");
...
export async function runDueReports(logger) {
    const now = new Date();
    ...
    for (const row of due) {
        try {
            ...
            const input = { employerId: row.employerId, frequency: row.frequency, timezone: row.timezone, sendTime: row.sendTime, dayOfWeek: row.dayOfWeek, dayOfMonth: row.dayOfMonth, onceDate };
