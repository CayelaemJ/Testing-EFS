...
import { computeOptimiseScore, DEFAULT_WEIGHTS } from "./scoreEngine.js";
...
const INCOME_VALUES = Object.keys(INCOME_LABEL);
const CORE_FEEDS = [
    "employers",
    "workforce_snapshots",
    "employees",
...
function previousQuarterClosePeriod({ year, q }) {
...
function resolveFilter(query = {}) {
    ...
    if (query.quarter && !quarter)
        throw new Error("quarter must be YYYY-Q1..YYYY-Q4");
    ...
    if (requestedAsAt && Number.isNaN(requestedAsAt.getTime()))
        throw new Error("asAt must be a valid ISO date");
    const asAt = period
    ...
    const income = query.income && INCOME_VALUES.includes(query.income)
        ? query.income
        : null;
    return {
        period,
...
function existsAt(sourceDeletedAt, _asAt) {
...
function employeeEligibleAt(employee, asAt) {
...
async function workforceAsAt(employerId, asAt) {
  ...
  `;
    ...
    return { rows: mapped, usedFallback: mapped.some((r) => r.workforceSource === "projection"), missingAsAtCount: 0, versionCount: mapped.filter((r) => r.workforceSource === "version").length };
}
function inWindow(date, filter) {
    return !!date && date <= filter.rangeEnd && (!filter.rangeStart || date >= filter.rangeStart);
}
function atOrBefore(date, asAt) {
    return !!date && date <= asAt;
}
function wellnessBand(score) {
    if (score == null)
        return "Unavailable";
    if (score >= 80)
        return "Strong";
...
function maxDate(values) {
    const valid = values.filter((value) => value instanceof Date && !Number.isNaN(value.getTime()));
    return valid.length ? new Date(Math.max(...valid.map((date) => date.getTime()))) : null;
}
async function feedCoverageForEmployer(employerId, evidence) {
    ...
    const syncByFeed = new Map();
    for (const cursor of cursors) {
        if (cursor.lastSuccessAt)
            syncByFeed.set(cursor.reportKey, cursor.lastSuccessAt);
    }
    const importByFeed = new Map();
    for (const batch of batches) {
        if (!CORE_FEEDS.includes(batch.reportKey))
            continue;
...
async function activeWeights(employerId, asAt) {
    const rows = await prisma.scoreWeight.findMany({
        where: { OR: [{ employerId }, { employerId: null }], effectiveFrom: { lte: asAt } },
        orderBy: { effectiveFrom: "desc" },
    });
    const pick = (driver) => rows.find((row) => row.driver === driver && row.employerId === employerId)
        ?? rows.find((row) => row.driver === driver && row.employerId == null);
    return {
...
async function latestDebtState(platformUserIds, asAt) {
    if (!platformUserIds.length)
        return { rows: [], usedFallback: false, versionCount: 0 };
    ...
    for (const version of versions)
        if (!latest.has(version.accountId))
            latest.set(version.accountId, version);
    const seen = new Set(latest.keys());
    const result = [];
    for (const version of latest.values()) {
        if (version.isDeleted || (version.closedAt && version.closedAt <= asAt))
            continue;
        result.push({
    ...
    for (const account of projections) {
        if (seen.has(account.id) || (account.closedAt && account.closedAt <= asAt))
            continue;
        usedFallback = true;
        result.push(account);
    }
...
async function latestPolicyState(platformUserIds, asAt) {
    if (!platformUserIds.length)
        return { rows: [], usedFallback: false, versionCount: 0 };
    ...
    const latest = new Map();
    for (const version of versions)
        if (!latest.has(version.policyId))
            latest.set(version.policyId, version);
    const seen = new Set(latest.keys());
    const result = [];
    for (const version of latest.values()) {
        if (version.isDeleted)
            continue;
        if (version.effectiveFrom && version.effectiveFrom > asAt)
            continue;
        if (version.effectiveTo && version.effectiveTo < startOfUtcDay(asAt))
            continue;
        result.push({
            id: version.policyId,
    ...
    let usedFallback = false;
    for (const policy of projections) {
        if (seen.has(policy.id))
            continue;
        if (policy.effectiveFrom && policy.effectiveFrom > asAt)
            continue;
        if (policy.effectiveTo && policy.effectiveTo < startOfUtcDay(asAt))
            continue;
        usedFallback = true;
...
async function buildDashboardPayload(employerId, query = {}, skipPrior = false) {
    ...
    const eligibleEmployees = workforceState.rows.filter((employee) => employeeEligibleAt(employee, filter.asAt));
    const cohortEmployees = eligibleEmployees.filter((employee) => (!filter.site || employee.site?.name === filter.site)
    ...
    const employeeByPlatformUser = new Map();
    for (const employee of cohortEmployees)
        if (employee.platformUser)
            employeeByPlatformUser.set(employee.platformUser.id, employee);
    ...
    const liveJourneys = journeys.filter((journey) => existsAt(journey.sourceDeletedAt, filter.asAt));
    ...
    const ratings = ratingsAll.filter((row) => existsAt(row.sourceDeletedAt, filter.asAt) && inWindow(row.createdAt, filter));
    const chats = chatsAll.filter((row) => existsAt(row.sourceDeletedAt, filter.asAt) && inWindow(row.startedAt, filter));
    const referrals = referralsAll.filter((row) => existsAt(row.sourceDeletedAt, filter.asAt) && inWindow(row.sharedAt, filter));
    const advances = advancesAll.filter((row) => existsAt(row.sourceDeletedAt, filter.asAt) && row.status === "FINALISED" && inWindow(row.advancedAt, filter));
    ...
    const feedCoverage = await feedCoverageForEmployer(employerId, {
        ...
        ratings: ratingsAll.length > 0,
        referrals: referralsAll.length > 0,
        salary_advances: advancesAll.length > 0,
    });
    ...
    const completedUserIds = new Set(cumulativeCompleted.map((journey) => journey.platformUserId));
    const completedFix = completedUserIds.size;
    const completedCounts = new Map();
    for (const journey of cumulativeCompleted)
        completedCounts.set(journey.platformUserId, (completedCounts.get(journey.platformUserId) ?? 0) + 1);
    const multipleFix = [...completedCounts.values()].filter((count) => count >= 2).length;
    const startedUserIds = new Set(liveJourneys.filter((journey) => journey.startedAt <= filter.asAt).map((journey) => journey.platformUserId));
    const activatedUsers = platformUsers.filter((user) => startedUserIds.has(user.id));
    const activated = activatedUsers.length;
    const cumulativeMonthlySaving = cumulativeCompleted.reduce((sum, journey) => sum + (journey.monthlySavingCents ?? 0), 0);
    const flowMonthlySaving = flowCompleted.reduce((sum, journey) => sum + (journey.monthlySavingCents ?? 0), 0);
    const arrearsUserIds = new Set(debts.filter((debt) => debt.inArrears).map((debt) => debt.platformUserId));
    const debtVisibleUserIds = new Set();
    for (const user of platformUsers)
        if (user.hasCreditProfile)
            debtVisibleUserIds.add(user.id);
    for (const debt of debts)
        if (debt.platformUserId)
            debtVisibleUserIds.add(debt.platformUserId);
    const wastefulPolicies = policies.filter((policy) => policy.isWasteful);
    const fixedPolicies = wastefulPolicies.filter((policy) => policy.isResolved);
    // Hoisted from further down (still used there): needed now to size the
    // cashflow "still achievable" denominator below.
    const countPolicy = (type, unresolvedOnly = false) => policies.filter((policy) => policy.type === type && policy.isWasteful && (!unresolvedOnly || !policy.isResolved)).length;
    const prescribable = debts.filter((debt) => ["IDENTIFIED", "LETTER_SENT"].includes(debt.challengeStatus));
    ...
    const cumulativeByType = new Map();
    for (const journey of cumulativeCompleted) {
    ...
    const overallAvgSavingCents = cumulativeCompleted.length
        ...
        : 0;
    const avgSavingForType = (type) => {
        const bucket = cumulativeByType.get(type);
        return bucket && bucket.count > 0 ? bucket.saving / bucket.count : overallAvgSavingCents;
    };
    const eligibleNotYetFixed = [
        ["ARREARS", debts.filter((debt) => debt.inArrears && (debt.state ?? "NONE") === "NONE").length],
    ...
    const additionalOpportunityCents = eligibleNotYetFixed.reduce((sum, [type, eligible]) => sum + eligible * avgSavingForType(type), 0);
    ...
    const target = targetRows.find((row) => row.employerId === employerId)
        ?? targetRows.find((row) => row.employerId == null);
    const computedAchievable = rand(cumulativeMonthlySaving) + rand(additionalOpportunityCents);
    const savingsAchievable = Number(target?.config?.monthlyAchievableRand ?? computedAchievable);
    const inputs = {
    ...
    const pct = (value) => headcount ? Math.round((value / headcount) * 100) : 0;
    const byType = new Map();
    for (const journey of flowCompleted) {
    ...
    const arrearsDebts = debts.filter((debt) => debt.inArrears);
    const arrearsTotal = arrearsDebts.reduce((sum, debt) => sum + debt.balanceCents, 0);
    const profileMap = new Map();
    for (const debt of arrearsDebts)
        profileMap.set(debt.creditType, (profileMap.get(debt.creditType) ?? 0) + debt.balanceCents);
    ...
    const stateAggregate = (state) => {
        const rows = debts.filter((debt) => debt.state === state);
        return {
            rand: rows.reduce((sum, debt) => sum + debt.balanceCents, 0),
            employees: new Set(rows.map((debt) => debt.platformUserId)).size,
        };
    };
    ...
    const avgRating = ratingResponses ? ratings.reduce((sum, rating) => sum + rating.stars, 0) / ratingResponses : 0;
    const ratingPct = {};
    for (let stars = 1; stars <= 5; stars++)
        ratingPct[stars] = ratingResponses ? Math.round(ratingDistribution[stars] / ratingResponses * 100) : 0;
    ...
    const challengeRows = debts.filter((debt) => debt.challengeStatus);
    const challengeOrder = ["IDENTIFIED", "LETTER_SENT", "CREDITOR_CONCEDED", "WRITTEN_OFF"];
    const stageCount = (stage) => challengeRows.filter((debt) => challengeOrder.indexOf(debt.challengeStatus) >= challengeOrder.indexOf(stage)).length;
    const challengedBalance = challengeRows.reduce((sum, debt) => sum + debt.balanceCents, 0);
    const writtenOffBalance = challengeRows.filter((debt) => debt.challengeStatus === "WRITTEN_OFF").reduce((sum, debt) => sum + debt.balanceCents, 0);
    const incomeColors = ["#003a66", "#0078c7", "#4ea3da", "#bfe0f5", "#8497a7"];
    const incomeCounts = new Map();
    for (const user of activatedUsers) {
        const employee = employeeByPlatformUser.get(user.id);
        if (!employee?.incomeBand)
            continue;
    ...
    const chatResolved = chats.filter((chat) => chat.resolvedInChat).length;
    const chatCsatRows = chats.filter((chat) => chat.satisfaction != null);
    const chatCsat = chatCsatRows.length ? chatCsatRows.reduce((sum, chat) => sum + chat.satisfaction, 0) / chatCsatRows.length : 0;
    const replyTimes = chats.filter((chat) => chat.firstReplySeconds != null).map((chat) => chat.firstReplySeconds).sort((a, b) => a - b);
    const medianReply = replyTimes.length ? replyTimes[Math.floor((replyTimes.length - 1) / 2)] : 0;
    const chatByType = new Map();
    ...
    for (const chat of chats) {
        ...
        bucket.convos++;
        if (chat.sentiment != null) {
            bucket.sentimentTotal += chat.sentiment;
            bucket.sentimentCount++;
        }
        if (chat.theme)
            bucket.themes.set(chat.theme, (bucket.themes.get(chat.theme) ?? 0) + 1);
    ...
    const sharers = new Set(referrals.map((referral) => referral.platformUserId)).size;
    const completedFlowUsers = new Set(flowCompleted.map((journey) => journey.platformUserId)).size;
    const channelCounts = new Map();
    for (const referral of referrals)
        if (referral.channel)
            channelCounts.set(referral.channel, (channelCounts.get(referral.channel) ?? 0) + 1);
    ...
    const ewaClients = new Set(advances.map((advance) => advance.clientId)).size;
    const ewaTotal = advances.reduce((sum, advance) => sum + advance.amountCents, 0);
    const ewaByMonth = new Map();
    for (const advance of advances)
        ewaByMonth.set(monthKey(advance.advancedAt), (ewaByMonth.get(monthKey(advance.advancedAt)) ?? 0) + advance.amountCents);
    ...
    const savingByMonth = new Map();
    for (const journey of cumulativeCompleted) {
        if (!journey.completedAt || !journey.monthlySavingCents)
            continue;
    ...
    for (const journey of liveJourneys) {
        const existing = firstActivatedAt.get(journey.platformUserId);
        if (!existing || journey.startedAt < existing)
            firstActivatedAt.set(journey.platformUserId, journey.startedAt);
    }
    let monthlySavingCum = 0;
    for (const [key, value] of savingByMonth.entries())
        if (key < trendMonths[0])
            monthlySavingCum += value;
    const takeUpTrend = [];
    const activatedTrend = [];
    const avgRatingTrend = [];
    const monthlySavingTrend = [];
    for (const month of trendMonths) {
        const cutoff = month === currentMonthKey ? filter.asAt : endOfMonth(month);
        takeUpTrend.push(pct(platformUsers.filter((u) => u.enrolledAt <= cutoff).length));
        activatedTrend.push(pct([...firstActivatedAt.values()].filter((d) => d <= cutoff).length));
        const ratingsToDate = ratingsAll.filter((r) => r.createdAt <= cutoff);
        avgRatingTrend.push(ratingsToDate.length ? Number((ratingsToDate.reduce((sum, r) => sum + r.stars, 0) / ratingsToDate.length).toFixed(1)) : 0);
    ...
    const trendDelta = (trend) => (trend.length >= 2 ? Number((trend[trend.length - 1] - trend[trend.length - 2]).toFixed(1)) : null);
    ...
    const outcomeMonthly = new Map();
    for (const journey of cumulativeCompleted) {
        if (!journey.completedAt)
            continue;
        const byMonth = outcomeMonthly.get(journey.type) ?? new Map();
    ...
    for (const [i, type] of [...byType.keys()].entries()) {
        const byMonth = outcomeMonthly.get(type) ?? new Map();
        outcomes[i].trend = trendMonths.map((m) => byMonth.get(m) ?? 0);
        outcomes[i].delta = trendDelta(outcomes[i].trend);
    }
    const regions = [];
    const stressMap = [];
    const siteStats = [];
    ...
    const sites = [...new Set([
            ...allSiteRows.map((row) => row.name),
            ...historicalSiteRows.map((row) => row.siteName),
            ...eligibleEmployees.map((employee) => employee.site?.name),
        ].filter(Boolean))].sort();
    for (const siteName of sites) {
        if (filter.site && filter.site !== siteName)
            continue;
        const siteEmployees = eligibleEmployees.filter((employee) => employee.site?.name === siteName && (!filter.income || employee.incomeBand === filter.income));
        const siteUserIds = new Set(siteEmployees.map((employee) => employee.platformUser?.id).filter(Boolean));
        const siteUsers = platformUsers.filter((user) => siteUserIds.has(user.id));
        if (!siteEmployees.length)
            continue;
        const siteDebts = arrearsDebts.filter((debt) => siteUserIds.has(debt.platformUserId));
        const siteArrearsUsers = new Set(siteDebts.map((debt) => debt.platformUserId)).size;
        const averageDebt = siteDebts.length ? siteDebts.reduce((sum, debt) => sum + debt.balanceCents, 0) / siteDebts.length : 0;
    ...
    const unresolvedPolicies = policies.filter((policy) => policy.isWasteful && !policy.isResolved);
    const unresolvedPremium = unresolvedPolicies.reduce((sum, policy) => sum + policy.premiumCents, 0);
    ...
    const prescribableBalance = prescribable.reduce((sum, debt) => sum + debt.balanceCents, 0);
    const opportunities = {
        cards: [
    ...
    if (!skipPrior && score.complete) {
        ...
        if (filter.period) {
            ...
            const priorPayload = await buildDashboardPayload(employerId, { range: "30d", asAt: priorEnd.toISOString(), ...cohort }, true);
        ...
        for (let offset = 0; offset < 25 && priorScore == null; offset++) {
            ...
            const priorPayload = await buildDashboardPayload(employerId, { period: candidate, ...cohort }, true);
            if (priorPayload?.wellness?.complete && priorPayload?.wellness?.score != null) {
                priorScore = Number(priorPayload.wellness.score);
            }
        }
    ...
    if (!headcountSnapshot && !filter.site && !filter.income && (feedCoverage.workforce_snapshots.available || feedCoverage.employees.available))
        warnings.push("No workforce headcount snapshot existed at the selected as-of date; the employee effective-date count/current cache was used.");
    ...
    if (workforceState.usedFallback)
        warnings.push("Some employees used the current workforce projection because no dated workforce observations were available; historical site/income cohorts should be backfilled.");
    if (workforceState.missingAsAtCount > 0)
        warnings.push(`${workforceState.missingAsAtCount} employee record(s) had dated workforce history but no valid observation at or before the selected as-of date.`);
    if (eligibleEmployees.some((employee) => !employee.eligibleFrom))
        warnings.push("Some legacy employee records have no eligible_from date; historical workforce cohorts may be incomplete until they are backfilled.");
    if (debtState.usedFallback)
        warnings.push("Some debt accounts used the current projection because no immutable observation history was available.");
    if (policyState.usedFallback)
        warnings.push("Some policies used the current projection because no immutable observation history was available.");
    if ((filter.site || filter.income) && (!employeeIds.length || headcount === 0))
        warnings.push("The selected cohort has no eligible employees at the as-of date.");
    const sourceDataUpdatedAt = maxDate([
        employer.sourceUpdatedAt,
        headcountSnapshot?.sourceUpdatedAt,
        ...cohortEmployees.map((employee) => employee.sourceUpdatedAt),
        ...platformUsers.map((user) => user.sourceUpdatedAt),
        ...liveJourneys.map((journey) => journey.sourceUpdatedAt),
        ...debts.map((debt) => debt.sourceUpdatedAt),
        ...policies.map((policy) => policy.sourceUpdatedAt),
        ...ratings.map((rating) => rating.sourceUpdatedAt),
        ...chats.map((chat) => chat.sourceUpdatedAt),
        ...referrals.map((referral) => referral.sourceUpdatedAt),
        ...advances.map((advance) => advance.sourceUpdatedAt),
    ]);
    ...
    return {
        ...
        monthActivity: {
            label: filter.label,
            enrolled: sectionAvailability.enrolment ? platformUsers.filter((user) => inWindow(user.enrolledAt, filter)).length : null,
...
async function readPersistentDashboardCache(employerId, query) {
...
async function persistDashboardCache(employerId, query, payload) {
    const cacheKey = dashboardCacheKey(employerId, query);
    const period = query.period ?? null;
...
export async function getDashboardPayload(employerId, query = {}) {
    ...
    if (query.period && query.range == null) {
        try {
            await persistDashboardCache(employerId, query, payload);
        }
        catch { /* cache failure must never break dashboard */ }
    }
...
async function persistScoreSnapshot(employerId, period, payload) {
    if (!payload?.wellness?.complete || payload?.wellness?.score == null)
        return false;
    const weights = {
    ...
    const rawScore = Number((sub.engagement * weights.ENGAGEMENT
        + sub.cashflow * weights.CASHFLOW
        + sub.debtRisk * weights.DEBT_RISK
        + sub.insurance * weights.INSURANCE).toFixed(2));
...
async function warmDashboardCohorts(employerId, periods) {
    const employees = await prisma.employee.findMany({
        where: { employerId, sourceDeletedAt: null },
        select: { incomeBand: true, site: { select: { name: true } } },
    });
    const sites = [...new Set(employees.map((e) => e.site?.name).filter(Boolean))];
    const incomes = [...new Set(employees.map((e) => e.incomeBand).filter(Boolean))];
    const queries = [];
    ...
    for (const period of periods) {
        for (const site of sites)
            queries.push({ period, site });
        for (const income of incomes)
            queries.push({ period, income });
        for (const site of sites)
            for (const income of incomes)
                queries.push({ period, site, income });
    }
    for (let i = 0; i < queries.length; i += 2) {
        await Promise.all(queries.slice(i, i + 2).map(async (query) => {
            ...
            catch { /* incomplete cohort stays cold */ }
        }));
    }
}
export async function snapshotEmployer(employerId, period = currentPeriod()) {
    if (!isValidPeriod(period) || period > currentPeriod())
        throw new Error(`invalid or future period: ${period}`);
    ...
    for (const k of [...dashboardCache.keys()])
        if (k.startsWith(`[\"${employerId}\"`))
            dashboardCache.delete(k);
    const payload = await buildDashboardPayload(employerId, { period });
...
async function monthsWithData(employerId) {
    const employees = await prisma.employee.findMany({ where: { employerId }, select: { id: true, platformUser: { select: { id: true } } } });
    const employeeIds = employees.map((employee) => employee.id);
    const platformUserIds = employees.map((employee) => employee.platformUser?.id).filter((id) => !!id);
    ...
    const add = (date) => { if (date)
        months.add(monthKey(date)); };
    for (const workforce of workforceVersions) {
        add(workforce.observedAt);
        add(workforce.eligibleFrom);
        add(workforce.eligibleTo);
    }
    for (const user of users) {
        add(user.enrolledAt);
        add(user.activatedAt);
    }
    for (const journey of journeys) {
        add(journey.startedAt);
        add(journey.completedAt);
    }
    for (const rating of ratings)
        add(rating.createdAt);
    for (const chat of chats)
        add(chat.startedAt);
    for (const referral of referrals) {
        add(referral.sharedAt);
        add(referral.convertedAt);
    }
    for (const advance of advances)
        add(advance.advancedAt);
    for (const headcount of headcounts)
        add(headcount.asOfDate);
    for (const debt of debtVersions)
        add(debt.observedAt);
    for (const policy of policyVersions)
        add(policy.observedAt);
    return [...months].sort();
}
export async function buildMonthlySnapshots(employerId, currentP = currentPeriod()) {
