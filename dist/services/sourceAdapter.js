...
function cleanIdentifier(value, label, allowBlank = false) {
...
function normalizeSqlRows(reportKey, rows) {
    ...
    return rows.map((row) => {
        ...
        for (const field of format.fields) {
            if (!lower.has(field.name.toLowerCase()))
                continue;
...
async function createApiAdapter(config) {
    ...
    async function fetchReport(reportKey, window) {
        ...
        if (!response.ok)
            throw new Error(`HTTP ${response.status} from ${reportKey} endpoint`);
        const body = await response.json();
        const records = Array.isArray(body) ? body : body?.records;
        if (!Array.isArray(records))
            throw new Error(`${reportKey}: response missing "records" array`);
...
function quoteSqlIdentifier(dialect, identifier) {
...
function qualifiedView(dialect, config, reportKey) {
    const { schema, view } = canonicalViewParts(config, reportKey);
    const qView = quoteSqlIdentifier(dialect, view);
    return schema ? `${quoteSqlIdentifier(dialect, schema)}.${qView}` : qView;
}
async function openPostgresExecutor(config) {
    const pg = await import("pg");
    const Client = pg.Client ?? pg.default?.Client;
    if (!Client)
        throw new Error("The pg driver could not be loaded.");
    ...
    return {
        ...
        async queryReport(reportKey, window, maxRows) {
            ...
            if (window.since) {
                ...
                where.push(`source_updated_at > $${params.length}`);
            }
            if (window.through) {
                params.push(window.through);
                where.push(`source_updated_at <= $${params.length}`);
            }
            ...
            return { rows: result.rows, columns: result.fields?.map((f) => String(f.name)) ?? [] };
        },
        async probe(reportKey) {
            const view = qualifiedView("POSTGRESQL", config, reportKey);
            const result = await client.query(`SELECT * FROM ${view} WHERE 1 = 0`);
            return result.fields?.map((f) => String(f.name)) ?? [];
        },
        async close() { await client.end(); },
    };
}
async function openMysqlExecutor(config) {
    const mysql = await import("mysql2/promise");
    const createConnection = mysql.createConnection ?? mysql.default?.createConnection;
    if (!createConnection)
        throw new Error("The mysql2 driver could not be loaded.");
    const queryTimeout = intSetting(config.sqlQueryTimeoutMs, 60_000, 1_000, 600_000);
    const sslEnabled = boolEnv("SOURCE_SQL_SSL") ?? config.sqlSsl ?? true;
    const trust = boolEnv("SOURCE_SQL_TRUST_SERVER_CERTIFICATE") ?? config.sqlTrustServerCertificate ?? false;
    ...
    return {
        ...
        async queryReport(reportKey, window, maxRows) {
            ...
            return {
                ...
                columns: Array.isArray(fields) ? fields.map((f) => String(f.name)) : [],
            };
        },
        async probe(reportKey) {
            const view = qualifiedView("MYSQL", config, reportKey);
            const [, fields] = await connection.query({ sql: `SELECT * FROM ${view} WHERE 1 = 0`, timeout: queryTimeout });
            return Array.isArray(fields) ? fields.map((f) => String(f.name)) : [];
        },
        async close() { await connection.end(); },
    };
}
async function openMssqlExecutor(config) {
    const mssql = await import("mssql");
    const ConnectionPool = mssql.ConnectionPool ?? mssql.default?.ConnectionPool;
    if (!ConnectionPool)
        throw new Error("The mssql driver could not be loaded.");
    const queryTimeout = intSetting(config.sqlQueryTimeoutMs, 60_000, 1_000, 600_000);
    ...
    return {
        ...
        async queryReport(reportKey, window, maxRows) {
            ...
            const where = [];
            if (window.since) {
                request.input("since", mssql.DateTimeOffset, window.since);
                where.push("source_updated_at > @since");
            }
            if (window.through) {
                request.input("through", mssql.DateTimeOffset, window.through);
                where.push("source_updated_at <= @through");
            }
...
function sqlDialect(config) {
    ...
    if (raw === "POSTGRES" || raw === "PG")
        return "POSTGRESQL";
    if (raw === "SQLSERVER" || raw === "SQL_SERVER")
        return "MSSQL";
...
async function createSqlAdapter(config) {
    if (!config.sqlHost)
        throw new Error("SQL host is required.");
    if (!config.sqlDatabase)
        throw new Error("SQL database/catalog is required.");
    if (!config.sqlUsername)
        throw new Error("SQL read-only username is required.");
    if (!(process.env.SOURCE_SQL_PASSWORD || config.sqlPassword))
        throw new Error("SQL password is required. Set it in Admin or SOURCE_SQL_PASSWORD.");
    const dialect = sqlDialect(config);
    const maxRows = intSetting(config.sqlMaxRowsPerReport, 250_000, 1, 2_000_000);
    let executor;
    if (dialect === "POSTGRESQL")
        executor = await openPostgresExecutor(config);
    else if (dialect === "MYSQL")
        executor = await openMysqlExecutor(config);
    else
        executor = await openMssqlExecutor(config);
    async function fetchReport(reportKey, window = {}) {
...
export async function createSourceAdapter(config) {
    const mode = configuredSourceMode(config);
    const effective = withEnvironment(config);
