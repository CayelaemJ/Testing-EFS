function restoreDate(value, fieldName, rowNumber) {
    if (value == null || value === "")
        return null;
 ...
 */
export function rehydrateStagedRows(format, stagedRows) {
    ...
    for (const [index, row] of stagedRows.entries()) {
        for (const field of temporalFields) {
            ...
            if (value == null || value === "")
                continue;
            row[field.name] = restoreDate(value, field.name, index + 1);
        }
    }
    return stagedRows;
}
export function dateMillis(value, fieldName = "date") {
    if (value instanceof Date) {
...
export function compareDateValues(left, right, fieldName = "date") {
