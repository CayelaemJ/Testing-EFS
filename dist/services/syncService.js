...
export async function saveConfig(patch) {
    const data = { ...patch };
    if (patch.authToken === "" || patch.authToken == null)
        delete data.authToken;
    if (patch.sqlPassword === "" || patch.sqlPassword == null)
        delete data.sqlPassword;
    if (patch.sourceMode)
        data.sourceMode = patch.sourceMode.toUpperCase();
    if (patch.sqlDialect)
        data.sqlDialect = patch.sqlDialect.toUpperCase();
    return prisma.integrationConfig.upsert({
        where: { id: "default" },
...
export function publicConfig(config) {
    const { authToken, sqlPassword, ...safe } = config;
...
export async function runSync(trigger = "manual") {
    ...
    try {
        ...
        if (adapter) {
            try {
                ...
                summary.sourceClose = { error: error?.message ?? String(error) };
                anyFailed = true;
            }
        }
    }
    for (const employerId of touchedEmployers) {
        try {
            ...
            if (r.persisted)
                notifyScoreChangeIfCurrentPeriod(employerId, r.period, r.period).catch(() => { }); // best-effort
        }
        catch (error) {
    ...
    await prisma.syncLog.update({
        where: { id: log.id },
        data: { status, finishedAt, summary: summary, note },
    });
...
export async function testConnection(patch = {}) {
    ...
    if (cleanPatch.authToken === "" || cleanPatch.authToken == null)
        delete cleanPatch.authToken;
    if (cleanPatch.sqlPassword === "" || cleanPatch.sqlPassword == null)
        delete cleanPatch.sqlPassword;
    const config = { ...saved, ...cleanPatch };
    let adapter = null;
    try {
        adapter = await createSourceAdapter(config);
        ...
        return { ok: false, error: error?.message ?? String(error), sourceMode: (() => { try {
                return configuredSourceMode(config);
            }
            catch {
                return "UNKNOWN";
            } })() };
