...
export async function requestPasswordReset(email) {
    ...
    if (!user || !user.active)
        return { ok: true }; // don't reveal account existence/status
    const rawSetupToken = randomBytes(32).toString("hex");
...
export async function listUsers() {
    ...
    return users.map((u) => ({
        ...
        employers: u.links.map((l) => ({ id: l.employer.id, name: l.employer.name })),
...
export async function updateUser(userId, patch) {
    ...
    const user = await prisma.user.update({
        where: { id: userId },
        data: {
            name: patch.name, role: patch.role, active: patch.active,
    ...
    if (patch.active === false)
        await destroyAllSessionsForUser(userId); // end access immediately
...
export async function deactivateSelf(userId, reason) {
    const user = await prisma.user.update({
        where: { id: userId },
...
export async function deleteUserPermanently(userId) {
    ...
    if (user.active)
        throw new Error("deactivate the account before deleting it permanently");
    await destroyAllSessionsForUser(userId);
...
export async function completeSetup(token, newPassword) {
    validatePassword(newPassword);
...
function sanitize(u) {
    return { id: u.id, email: u.email, name: u.name, role: u.role, active: u.active, employers: (u.links ?? []).map((l) => l.employerId) };
}
export { prisma };
