...
async function commitRowsChunk(
  ...
  async function getEmployeeMap(rowsForChunk: Record<string, any>[]) {
    const keys = [...new Set(
      rowsForChunk
  ...
  switch (reportKey) {
    ...
    case "platform_users":
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!emp) throw new Error(`Row ${i + 1}: Employee ${key} not found. Import employees.csv first.`);
    ...
    case "debt_accounts":
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!emp?.platformUserId) throw new Error(`Row ${i + 1}: Platform user ${key} not found. Import platform_users.csv first.`);
...
export async function commitBatch(batchId: string) {
  ...
  if (batch.status !== "VALIDATED") throw new Error(`Batch status is ${batch.status}, must be VALIDATED`);

  const allRows = (batch.stagedRows ?? []) as Record<string, unknown>[];
  if (!allRows.length) throw new Error("No rows staged for commit");
